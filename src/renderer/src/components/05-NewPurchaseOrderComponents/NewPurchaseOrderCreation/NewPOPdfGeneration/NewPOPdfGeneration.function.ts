...
export const generatePurchaseOrderPdf = (
  ...
  // Construct supplier full address dynamically
  const supplierAddressParts = [
    ...
    from.supplierEmail ? `Email: ${from.supplierEmail}` : '',
    from.supplierContactNumber ? `Ph: ${from.supplierContactNumber}` : ''
  ].filter(Boolean)

  let addressY = 75
  supplierAddressParts.forEach((line) => {
  ...
  autoTable(doc, {
    ...
    body: [
      ['IGST', `${TAX_RATE}%`, `\u20B9 ${SUBTOTAL.toFixed(2)}`, `\u20B9 ${TAX_AMOUNT.toFixed(2)}`]
    ],
    styles: { fontSize: 9, lineWidth: 0 },
    headStyles: { fontStyle: 'bold', textColor: [0, 0, 0] }
  })
